...
func (s *SignalClient) getGroupInfo(ctx context.Context, groupID types.GroupIdentifier, minRevision uint32, backupChat *store.BackupChat) (*bridgev2.ChatInfo, error) {
	...
	avatar, err := s.makeGroupAvatar(ctx, groupID, groupInfo)
	if err != nil {
		return nil, err
	}
...
func (s *SignalClient) makeGroupAvatar(ctx context.Context, groupID types.GroupIdentifier, meta signalmeow.GroupAvatarMeta) (*bridgev2.Avatar, error) {
	...
	if s.Main.useDirectMedia {
		groupIDBytes, err := groupID.Bytes()
		if err != nil {
			return nil, err
		}
		mediaID, err := signalid.DirectMediaInfo{
			Type:            signalid.DirectMediaGroupAvatar,
			GroupID:         groupIDBytes[:],
			GroupAvatarPath: *meta.GetAvatarPath(),
		}.AsMediaID()
		if err != nil {
			return nil, err
		}
...
func (s *SignalClient) groupChangeToChatInfoChange(ctx context.Context, groupID types.GroupIdentifier, rev uint32, groupChange *signalmeow.GroupChange) (*bridgev2.ChatInfoChange, error) {
	avatar, err := s.makeGroupAvatar(ctx, groupID, groupChange)
	if err != nil {
		return nil, err
	}
